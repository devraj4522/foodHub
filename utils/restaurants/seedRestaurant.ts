  ...
  for (const restaurant of restaurantsData) {
    await prisma.restaurant.create({
      data: {
        ...
        openingTime: `${Math.floor(Math.random() * 12)}:${Math.floor(Math.random() * 60).toString().padStart(2, '0')}`,
        closingTime: `${Math.floor(Math.random() * 12) + 12}:${Math.floor(Math.random() * 60).toString().padStart(2, '0')}`,
